...
namespace StudCity.Application.CommandHandlers.Authenticate;
    ...
    : IRequestHandler<GoogleAuthenticateCommand, AuthenticateResponseModel>
{
    ...
    public async Task<AuthenticateResponseModel> Handle(GoogleAuthenticateCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (account == null)
        {
            ...
            var newAccount =
                await _db.Accounts.FirstAsync(x => x.Id == accountId, cancellationToken: cancellationToken);

            newAccount.AuthType = Core.Enums.AuthType.Google;
            newAccount.AccountRoles = new List<AccountRole>
            {
                new()
                {
                    ...
                    Role = _db.Roles.First(x => x.Id == Core.Enums.Role.User),
                },
            };
        ...
        if (account.AuthType != Core.Enums.AuthType.Google)
        {
