...
function HeaderEditProfile({ user, setUser, handleSave, handleCancel }) {
	...
	const styleForButton = 'mt-auto text-base max-w-[100px] h-10 mt-auto mx-0';
	const [avatar, setAvatar] = useState(user.avatar || DEFAULT_AVATAR_URL);
	const [backgroundImage, setBackgroundImage] = useState(
		user.settings.backgroundImage || DEFAULT_BACKGROUND_URL
	);
		...
		<div className='w-full p-2'>
			...
			<div className='h-32 w-full -mt-10 flex justify-between'>
				<div className='h-fit w-fit relative mx-auto'>
					<Avatar src={avatar} className={'w-28 h-28 shadow-form '}>
						<Button
							className={
								'absolute bottom-5 -right-2 h-8 w-8 rounded-full z-40 text-base text-center'
							}
				...
				<div className='w-9/12 h-full flex'>
					<div className='flex flex-col w-1/2 h-5/6'>
						...
						<h4 className='text-sm font-normal'>
							{user.description || t('profile.details.update_your_details')}
						</h4>
					</div>
					<div className='flex w-1/2 h-5/6 justify-end'>
						...
						<Button
							className={`${styleForButton} mx-5 w-fit px-2`}
							onClick={handleSave}
						>
							{t('base.save')}
						</Button>
