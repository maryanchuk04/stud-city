		...
		<div className='flex flex-col gap-3 w-24 items-center pt-2 border-r-2 h-full relative'>
			{
				ICON_NAVBAR_ICONS.map((item, index) => (
					<div
						...
						className={`w-12 h-12 flex cursor-pointer hover:bg-customGreen hover:shadow rounded-lg duration-300 ${index === active && "bg-customGreen shadow"}`}
					>
						<i className={`${item.icon} w-11/12 h-full m-auto grid place-items-center text-xl `}></i>
					</div>
				))
			}
			<button className='hover:bg-primaryGold duration-500 focus-visible:outline-none w-10 h-10 rounded-full bg-customGreen grid place-items-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2'
				onClick={handleClick}
			>
