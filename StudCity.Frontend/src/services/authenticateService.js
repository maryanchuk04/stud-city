...
export class AuthenticateService {
    ...
    async registration({ email, password }) {
        try {
            ...
            if (registrationResponse.status === STATUS_OK) return registrationResponse.data.accountId;
        } catch (err) {
            showAlert(err.response.data.error, 'error');
            return;
        }
     ...
     */
    async googleLogin({ name, picture, email, hd } ) {
        const { data } = await this.service.post(`${this.authUrl}/google`, { name, picture, email, hd });

        if (data.token) {
            this.tokenService.setToken(data.token);
            return { isRegistration: data.isRegistration, isSuccess: true };
        }
