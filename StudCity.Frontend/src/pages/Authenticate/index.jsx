...
function Authenticate() {
	...
	const handleSubmit = async (event) => {
		...
		if (isOk) {
			...
			navigate('/profile');
		} else setFormState({ email: '', password: '' });
		...
		<div className='w-1/1 h-screen flex'>
			<div className='w-1/2 h-screen flex bg-primatyWhite'>
				<div className='container flex flex-col w-2/3 m-auto h-4/5 justify-between bg-primatyWhite rounded-3xl p-11 py-20 shadow-md '>
					<h1 className='w-full text-center font-bold text-4xl text-primaryAuthentication'>
						{t('login.title')}
					</h1>
					<form className='flex flex-col w-1/1' onSubmit={handleSubmit}>
						...
						<Link
							to='/forgot-password'
							className='w-full text-center ml-1 font-medium text-base text-primaryAuthentication'
							href=''
						>
					...
					<div className=' w-full text-center ml-1 mt-3 font-medium text-base'>
						<Link to='/registration' className='mr-1 cursor-pointer text-[#233a2a]'>
			...
			<div className='w-1/2 h-screen '>
				<img
					...
					src='/images/Image-Authenticate-Light.jpg'
					alt='404'
				/>
			</div>
		</div>
